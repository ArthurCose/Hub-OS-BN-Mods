...
function create_pulse(user, props, audio)
	...
	spell:set_hit_props(
		HitProps.new(
			props.damage,
			Hit.Impact | Hit.Flinch | Hit.Drag | Hit.PierceInvis,
			props.element,
			user:context(),
	...
	local hitbox_1 = SharedHitbox.new(spell, 11)
	hitbox_1:set_hit_props(spell:copy_hit_props())
	local hitbox_2 = SharedHitbox.new(spell, 11)
	hitbox_2:set_hit_props(spell:copy_hit_props())
	local hitbox_3 = SharedHitbox.new(spell, 11)
