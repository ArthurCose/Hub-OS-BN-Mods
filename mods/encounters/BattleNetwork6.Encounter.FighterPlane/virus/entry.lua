...
function get_endloop_tile(plane)
    local y = plane:current_tile():y()
    if plane:rank() ~= Rank.V1 then
        local target = find_best_target(plane)
        if target and not target:deleted() then y = target:current_tile():y() end
    end
    ...
    spell.on_update_func = function(self)
        if flash_before_strike <= 0 then
            ...
            if do_once then
                ...
                Field.spawn(blast_fx, self:current_tile())
            end
        else
...
function character_init(plane)
    ...
    plane.on_spawn_func = function(self)
        ...
        plane:show_shadow();
        self._original_tile = self:current_tile()
    ...
    local on_collision_func = function(self, other)
        if Obstacle.from(other) ~= nil then
            if not self:is_sliding() and self:current_tile():is_edge() and not self:is_teleporting() and not self._is_looping then
    ...
    plane.on_update_func = function(self)
        ...
        if self:rank() == Rank.NM and self:health() <= 100 then
            ...
            if self:current_tile() ~= self._original_tile and not self:is_sliding() then
                self:slide(self:get_tile(self:facing(), 1), (plane._tile_slide_speed), function()
                    self._spawned_hitbox = false
                    self._do_once = true
                    self._is_looping = false
                end)
            elseif self:current_tile() == self._original_tile and not self:is_sliding() then
            ...
            if not self._has_risen then
                ...
                if self._nm_track_once then
                    ...
                    if target and not target:deleted() then
                        self._nm_divebomb_tile = target:current_tile()
                    end
                    target_tile = self._nm_divebomb_tile
                    self._nm_track_once = false
                end
                local speedy_boy = (math.abs(self._target_height * ((target_tile:x() - self:current_tile():x()) + (target_tile:y() - self:current_tile():y()))) / 60)
            ...
            if self._current_move_delay <= 0 then
                if self._should_shoot then
                    if self._do_once then
                        ...
                        for i = 1, Field.width(), 1 do
                            ...
                            if tile:is_edge() or tile:team() == team then goto continue end

                            if tile:x() > self:current_tile():x() and self:facing() == Direction.Right or tile:x() < self:current_tile():x() and self:facing() == Direction.Left then
                                if i % 2 == 0 then
                ...
                elseif self._should_move then
                    if #self:current_tile():find_characters(character_query) > 0 and not self._spawned_hitbox then
                        ...
                        self:field():spawn(hitbox, self:current_tile())
                    ...
                    if self._movement_loops > 0 then
                        ...
                        if self:current_tile() ~= self._original_tile then
                            self:slide(self:get_tile(self:facing(), 1), (plane._tile_slide_speed), function()
                                self._is_looping = false
                                self._spawned_hitbox = false
                            end)
