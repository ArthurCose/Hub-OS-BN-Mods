...
function spawn_attack(user, props, state, offset_list, is_slide, is_pierce)
	...
	if state == "SONIC_FIRE" then props.secondary_element = Element.Fire end
	if state == "SONIC_AQUA" then props.secondary_element = Element.Aqua end
	if state == "SONIC_ELEC" then props.secondary_element = Element.Elec end
