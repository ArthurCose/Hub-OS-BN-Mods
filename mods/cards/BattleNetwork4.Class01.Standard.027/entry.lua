...
local AUDIO = Resources.load_audio("IceBreath.ogg")
...
function card_init(actor, props)
    ...
    action.on_execute_func = function(self, user)
        ...
        buster_anim:set_state("SPAWN_ICE")
        ...
        buster_anim:on_frame(2, function()
            buster_anim:set_state("ATTACK_ICE")
...
function create_attack(user, props, spawn_tile, first)
    ...
    spell.on_update_func = function(self)
        ...
        spell:current_tile():set_state(TileState.Ice)



        if time == 15 then
            anim:set_state("DESPAWN_ICE")
