...
local function resolve_gust_x(team, direction, y)
  -- resolve closest tile, starting at farthest tile
  -- this helps us properly resolve for x when we're surrounded
  ...
  for x = start_x, end_x, inc_x do
    ...
    if tile:team() ~= team then
      found_enemy_team = true
      gust_x = x
    elseif found_enemy_team then
      -- avoid spawning on team tiles
      -- we wait until we've seen an enemy team panel
      -- in case our enemy is in the center on surrounded terrain
      break
    end
  end
...
function card_init(user)
  ...
  action.on_execute_func = function()
    ...
    local function normal_update()
      ...
      -- spawn gust
      gust = gust_builder:create_spell(team, direction)
      gust:set_texture(GUST_TEXTURE)
      gust:set_facing(direction)
