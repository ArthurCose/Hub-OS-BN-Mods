...
local AUDIO = Resources.load_audio("HeatBreath.ogg")
...
function card_init(actor, props)
    ...
    action.on_execute_func = function(self, user)
        ...
        buster_anim:set_state("SPAWN_FIRE")
        ...
        buster_anim:on_frame(2, function()
            buster_anim:set_state("ATTACK_FIRE")
...
function create_attack(user, props, spawn_tile, first)
    ...
    spell:current_tile():set_state(TileState.Lava)
    ...
    spell.on_update_func = function(self)
        ...
        if tileCheck == 0 and spell:current_tile():is_walkable() then
            spell:current_tile():set_state(TileState.Lava)
            tileCheck = 1;
        end
