...
function card_init(user, props)
    ...
    action.on_execute_func = function(self, user)
        ...
        self:on_anim_frame(2, function()
            ...
            local up_attack = create_attack(user, props)
            local down_attack = create_attack(user, props)
			...
			if up_tile then
				Field.spawn(up_attack, up_tile)
			end
			local down_tile = tile:get_tile(Direction.Down, 1)
			if down_tile then
				Field.spawn(down_attack, down_tile)
			end
        end)
