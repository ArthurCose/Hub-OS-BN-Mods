...
function card_init(user, props)
	...
	action:set_lockout(ActionLockout.new_async(1))
...
function create_puck(user, props)
	...
	spell:set_hit_props(
		HitProps.new(
			props.damage,
			Hit.Impact | Hit.Flinch | Hit.PierceGuard | Hit.Shake,
			props.element,
			user:context(),
			Drag.None
		)
	...
	spell.on_collision_func = function(self, other)
		...
		fx:animation():load("artifact_impact_fx.animation")
		fx:animation():set_state("BLUE")
	...
	spell.on_delete_func = function(self)
		if not spell:current_tile():is_edge() then
			...
			fx:animation():load("mob_move.animation")
